  ...
  const fetchOrders = async () => {
    try {
      ...
      if (response && response.success) {
        ...
        toast.error('Failed to load orders: ' + ((response && response.message) || 'Unknown error'));
      ...
      <div className="w-full">
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          ...
          {ordersState.loading ? (
            ...
            <>
              {!Array.isArray(ordersState.filtered) || ordersState.filtered.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {ordersState.filtered.map((order, index) => (
                        <tr key={order._id || index} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {order._id && order._id.length > 8 ? `${order._id.substring(0, 8)}...` : (order._id || 'N/A')}
                          </td>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-900">{orderDetails._id || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-sm">Invoice Number</p>
                        <p className="text-gray-900">{orderDetails.invoiceNumber || 'N/A'}</p>
                      </div>
                      <div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="mb-3">
                      ...
                      <p className="text-gray-900">{orderDetails.mobileNo || 'N/A'}</p>
                    </div>
                    <div className="mb-3">
                      <p className="text-gray-500 text-sm">Delivery Address</p>
                      <p className="text-gray-900">{orderDetails.deliveryAddress || 'N/A'}</p>
                      <p className="text-gray-900">{orderDetails.district || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 text-sm">Payment Method</p>
                      <p className="text-gray-900">{orderDetails.paymentMethod || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-gray-50 rounded-lg overflow-hidden">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {Array.isArray(orderDetails.orderItems) && orderDetails.orderItems.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900 text-right">{item.qty || 0}</td>
                          <td className="px-6 py-4 text-sm font-medium text-gray-900 text-right">
                            {item.price && item.qty ? `Rs.${(item.price * item.qty).toLocaleString()}` : 'N/A'}
                          </td>
                        </tr>
                      ))}
                    ...
                    <tfoot className="bg-gray-50">
                      <tr>
                        ...
                        <td className="px-6 py-4 text-sm font-medium text-gray-900 text-right">
                          {orderDetails.totalPaid ? `Rs.${orderDetails.totalPaid.toLocaleString()}` : 'N/A'}
                        </td>
                      </tr>
                      <tr>
